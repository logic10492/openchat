...
5. 2026-05-17 Background Source Tools Phase 4A-4D 已落地：Memory / WorldBook read-only recall tools 与 BackgroundSource adapters 已通过 focused closeout；BackgroundWorker 仍记录为 AgentCore 受限 consumer，但 `BackgroundWorker`、`BackgroundPacket`、`BackgroundManager`、`BackgroundAssembler` 和 Chat/Prompt switch 仍未实现。
...
- 当前源码现实仍保持：Memory 和 WorldBook 由 Chat 兼容链路消费，Prompt 输出仍是 `[Memories]` / `[World Book Entries]`；`BackgroundWorker` / `BackgroundPacket` / Chat-Prompt switch 未实现。
