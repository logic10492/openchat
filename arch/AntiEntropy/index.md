...
| Triangle-Consistency | [triangle-consistency.md](triangle-consistency.md) | `src-test` 最近 full suite 通过 378 tests / 67 suites；Prompt 四层顺序、数据库迁移约束、Memory、WorldBook Vectorization、AgentCore、Background Source/Worker、Director contract 与 Stage runtime foundation 已回写，Feature 分层漂移和 UI 自动化留待单独修复计划。 |
...
最新 full suite 结果：`xcodebuild test -project OpenChat.xcodeproj -scheme OpenChat -destination 'platform=iOS Simulator,id=F8D0D88B-71FD-471F-855A-B2B5D8267117'` 在 iOS 26.5 `iPhone 17 Pro` 上成功，Swift Testing `378 tests in 67 suites passed`，结尾为 `** TEST SUCCEEDED **`。当前基线已覆盖 API-client、Prompt 四层顺序、Memory embedding/vector/retrieval/extraction-cutoff/recall-trace/fallback-tier/retain-v2-provenance/reflect-contract、Responses request-shape、checkpoint compression、compression mode、WorldBook Vectorization Phase A/B/C/D、AgentCore foundation、Background source tools / worker / prompt switch、Director contract 和 Stage runtime foundation tests。
