...
| Triangle-Consistency | [triangle-consistency.md](triangle-consistency.md) | `src-test` 当前通过 133 tests；Prompt 时间格式、记忆/世界书顺序、数据库迁移约束、测试数量与覆盖范围已回写，Feature 分层漂移留待单独修复计划。 |
...
结果：`xcodebuild test` 成功，Swift Testing `133 tests in 28 suites passed`，结尾为 `** TEST SUCCEEDED **`。API-client 对齐测试已在后续主线整理中纳入提交。
