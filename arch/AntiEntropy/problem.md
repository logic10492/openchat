...
| ~~P2~~ | ~~fallback 和距离阈值缺少校准与可观测性~~ | ~~用户可能看到"不相关记忆被塞入"或"相关记忆没命中"，且 UI 难以解释~~ | 已修复到 Memory 内部 trace：`MemoryRecallTrace` 记录候选数、fallback reason、selected ids 和 omitted；distance threshold 丢弃进入 omission。UI 展示仍未实现。 | **Closed** |
...
| `arch-test` | 大部分已修复 | cutoff 边界、提取 phase、migration、发送链路持久 sortOrder 触发、PromptAssembler memory trim 保序、fallback tiers、recall trace、provenance migration、v2 parsing、dedupe、source validation、atomic write、reflect contract、Responses request shape 已有测试覆盖。UI 自动化和 Background block request-shape 仍待后续计划。 |
